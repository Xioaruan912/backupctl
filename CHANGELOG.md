...
- `backupctl run --project <id>` 参数解析错误（把项目 ID 当成未知参数）。
- 单项目备份不再更新 `last-success`、不执行历史清理、远端标记为 `RUN_PARTIAL`
  而不是 `RUN_COMPLETE`（状态显示为 `PROJECT_ONLY`）。
- 清理达到「最少保留数量」时不再误报为警告/退出码 10。
- `status` / `check` 按进程存活判断锁状态，不再把陈旧锁信息显示为「有任务运行中」。
